...
function setupTestDatabase() {
  ...
  // Mirror the production DDL — including unban + actor — so the drizzle
...
describe('Unban event recording', () => {
  ...
  it('inserts a row with unban=true, scenario=crowdsieve/unban and the supplied actor', async () => {
    ...
    const row = sqlite
      .prepare('SELECT scenario, unban, actor, message, source_ip, filtered FROM alerts WHERE id = ?')
      .get(id) as {
        scenario: string;
        unban: number;
        actor: string | null;
    ...
    // Defensive: unban events are pre-flagged as filtered so they never get
...
describe('Stats exclude unban events; listings include them', () => {
  ...
  it('counts only real alerts in getStats / getTimeDistributionStats and exposes both in queryAlerts', async () => {
    ...
    expect(unbanRow?.unban).toBe(true);
